...
#example:
#bash ihostmod.sh wlan0 eth0 : smartphone connect to wlan0(hostapd), ihost uplink through eth0 (dhcp required), no wlan sniffer,no lan sniffer
...
#bash ihostmod.sh x x wlan0 eth0 : wlan sniffer on wlan1, lan sniffer on eth0 (ihost with a wireless router)
#bash ihostmod.sh x x 172.16.0.10/wlan100  eth0 : wlan sniffer on rpcap://172.16.0.10/wlan100, lan sniffer on eth0(ihost with a ruckus ap)
#bash ihostmod.sh x x wlan0 wlan0 : wlan sniffer on wlan1, lan sniffer on wlan1(ihost without a wireless router or ruckus ap)
...
then
    ...
    WLCMD="wlcap -l -i $WLAN_SNIF_IF -T fields -E separator=,  -e wlan.fc.type_subtype -e wlan.sa -e frame.time_epoch -e frame.protocols -e radiotap.dbm_antsignal -e ppi.80211-common.dbm.antsignal -e wlan_mgt.ssid -R \"wlan.fc.type_subtype==0x04\""
