...
#example:
#bash ihostmod.sh wlan0 p1p1dhcp : smartphone connect to wlan0(hostapd), ihost uplink through p4p1 (dhcp required), no wlan sniffer,no lan sniffer
#bash ihostmod.sh p1p1 p1p1 : smartphone connect to outside router, and then to ihost through p4p1(static ip), no wlan sniffer,no lan sniffer
#bash ihostmod.sh p4p1 p1p1 : smartphone connect to outside router, and then to ihost through p4p1(static ip), and then to p1p1(static ip) -> internet
#bash ihostmod.sh p4p1 p1p1dhcp : smartphone connect to outside router, and then to ihost through p4p1(static ip), and then to p1p1(dynamic ip) -> internet
...
put_p4p1_connect_to_startup(){
cat >> ./startup.sh << EOF
# connect p4p1 (dynamic ip)
ifconfig p4p1 up
dhclient p4p1
...
put_p4p1_0_connect_to_startup(){
cat >> ./startup.sh << EOF
# connect p4p1 (static ip)
ifconfig p4p1 up
ip addr add 192.168.100.200/24 dev p4p1
route add -net 0.0.0.0/0 gw 192.168.100.100
EOF
}

put_p4p1_1_connect_to_startup(){
cat >> ./startup.sh << EOF
# connect p4p1:10
ifconfig p4p1:10 up
...
put_p1p1_1_connect_to_startup(){
cat >> ./startup.sh << EOF
# connect p1p1 (static ip)
ifconfig p1p1 up
ip addr add 192.168.100.200/24 dev p1p1
route add -net 0.0.0.0/0 gw 192.168.100.100
EOF
}
...
cat >> ./startup.sh << EOF
# remove 172.16.0.1 from p4p1
ip addr del 172.16.0.1/16 dev p4p1
...
then
    ...
    then
        ...
        echo "iw dev wlan0 interface add mon.ihost type monitor" >> ./startup.sh
